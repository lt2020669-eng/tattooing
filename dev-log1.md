...
- 已补充支付说明文案，当前会明确展示：
  - 总价为参考总价
  - Calendly 预约时先支付 `12.59 USD` 定金（约 `1,999 JPY`）
  - 定金支付成功后发送问卷
  - 纹身完成后在店内支付尾款
  - USD -> JPY 为估算值，最终以支付平台结算为准
  - 支付定金即视为同意 Calendly 付款页展示的预约条款

### 4) Process 流程与支付顺序改版（已上线）

- `Process` 区块已从旧顺序：
  - `预约 -> 问卷 -> 定金 -> 到店`
- 调整为当前实际顺序：
  - `选择时间并支付定金 -> 定金后发送问卷 -> 到店前确认方案 -> 到店施作并支付尾款`
- 当前问卷信息方向已在文案中明确：
  - 喜好与风格偏好
  - 是否自带图案
  - 是否需要设计师协助设计
  - 是否年满 `18` 岁
  - 纹身对用户的意义
- `Booking` 区块提示文案也已同步更新，说明打开预约页后会进行：
  - 选择时间
  - 支付定金
  - 查看预约条款
- 本次更新目标是让用户在进入 Calendly 前，先理解付款顺序、问卷时机与尾款规则，减少预约误解
...
1. 为 `Privacy` / `Terms` 补真实页面或真实目标，并让 `Price` / `Process` 中提到的条款有可落地入口
2. 如后续要减少支付争议，可进一步明确“定金按固定 `1999 JPY` 抵扣”还是“按实际汇率折算抵扣”
3. 继续补充更多真实作品与故事，沿用 `stories.json + storieslanguage.json` 模型
4. 视上线需要补充 `sitemap.xml`、`robots.txt`、OG 标签
